function S = jp_spm8_smooth(S, subnum, sessionnum)
%JP_SPM8_SMOOTH Smooth images using SPM8.
%
% S = JP_SPM8_SMOOTH(S, SUBNUM, [SESSIONNUM]) will smooth images
% for subject number SUBNUM from an S structure (see JP_INIT).
%
% Required option in S.cfg.jp_spm8_smooth is the fwhm field.  Other
% options:
...
if ~isfield(cfg, 'fwhm') || isempty(cfg.fwhm)
  jp_log(errorlog, 'Must specify S.cfg.jp_spm8_smooth.fwhm.', 2);
end
...
for thisp=1:size(P,1)
  ...
  spm_smooth(thisFile,fullfile(pathstr,sprintf('s%s.nii',name)),cfg.fwhm);
