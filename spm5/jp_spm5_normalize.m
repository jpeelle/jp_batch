function S = jp_spm5_normalize(S, subnum, sessionnum)
%JP_SPM5_NORMALIZE Normalize images using SPM5.
%
% S = JP_SPM5_NORMALIZE(S, SUBNUM, [SESSIONNUM])will normalize images
% for subject number SUBNUM from an S structure (see JP_INIT).
%
% Options available in S.cfg.jp_spm5_normalize include:
...
str = sprintf('Writing images: %s/%s',sprintf('%%%ii',ndigits),sprintf('%%%ii',ndigits));
